...
class SecurePrefs(
  ...
  /**
   * Auto-configure gateway settings and mark onboarding as completed.
   * Called once at app startup if onboarding hasn't been completed yet.
   * This allows the app to skip the onboarding UI entirely.
   */
  fun autoConfigureIfNeeded(): Boolean {
    if (_onboardingCompleted.value) return false
    val autoHost = "192.168.21.84"
    val autoPort = 18789
    val autoToken = "8be19321634a56d832bb89e93379c3860"
    // Don't set manualEnabled here — autoConnect() will do it right before connectManual()
    // This prevents NodeRuntime's discovery autoconnect from firing without the token
    setManualHost(autoHost)
    setManualPort(autoPort)
    setManualTls(false)
    setGatewayToken(autoToken)
    setGatewayBootstrapToken("uKx2-q9VV7ZKz29nuu8yWJWPMMeFqOhDCAuIJWcPt-g")
